...
from kbert.tokenizer.utils import molecules_from_texts, group_by_index, mask_array_with_minus_1, \
    get_input_ids_by_statement_by_role_by_molecule, add_statement_texts


def extend_index(self, index_file):
    index_extension = pd.read_csv(index_file, index_col=0, names=['text'])
    index_extension['tokens'] = self.batch_encode_plus_old(
...
# monkey patch for method transformers.tokenization_utils_base.PreTrainedTokenizerBase.batch_encode_plus
def batch_encode_plus(
    ...
    # get molecules
    molecules = molecules_from_texts(batch_text_or_text_pairs)
    # get targets
    ...
    statement_offset_by_molecule = self.get_target_and_statement_token_ids(molecules, statements, targets, max_length)
    ...
    input_ids[cls_tokens_y, cls_tokens_x] = self.cls_token_id
...
def get_target_and_statement_token_ids(self, molecules, statements, targets, max_length):
